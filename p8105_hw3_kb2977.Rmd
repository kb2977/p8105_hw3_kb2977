...
* The dataset consists of `r nrow(instacart)` rows and `r ncol(instacart)` columns.
* The average number of items ordered in this dataset is
```{r}
instacart %>% 
  group_by(order_id) %>% 
  count(order_id) %>% 
  summarize(ave_item = mean(n))
```
* The user_id with the most orders was `r names(which.max(table(instacart$user_id)))`.
* The item that was the most ordered was `r names(which.max(table(instacart$product_name)))`.
* The item that was the most reordered was `r instacart %>% group_by(product_name) %>%  names(which.max(table(instacart$product_name)))`.
