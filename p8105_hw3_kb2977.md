...
-   Customers tend to place orders around `names(which.max(table(instacart$product_name)))` hour of the day.
...
ave_temp = 
  ...
  ggplot(aes(x = id, y = ave_max_temp)) +
  geom_hex() +
  facet_grid(~month) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), text = element_text(size = 7)) +
  labs(
...
snowfall_plot = 
  ...
  geom_density_ridges(alpha = .5, color = "blue") +
  labs(
